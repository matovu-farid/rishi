...
class ContinuousViewManager extends _default2.default {
  ...
  /**
   * Checks if new views need to be added based on current scroll position
   * and adds them to maintain continuous scrolling experience.
   *
   * This method is the core of the continuous view manager's infinite scrolling
   * functionality. It determines if the user has scrolled near the beginning
   * or end of the current content and adds new sections accordingly.
   *
   * @param {number} [_offsetLeft] - Optional horizontal offset override
   * @param {number} [_offsetTop] - Optional vertical offset override
   * @returns {Promise<boolean>} - Resolves to true if new views were added, false otherwise
   *
   * @description
   * The method works by:
   * 1. Calculating the current scroll position and viewport dimensions
   * 2. Determining if user is near the start (needs prepending) or end (needs appending)
   * 3. Adding new sections from the book's navigation (prev/next)
   * 4. Displaying the new views and recursively checking if more are needed
   * 5. Updating the view visibility after all new views are rendered
   *
   * @example
   * // Check if new views are needed at current scroll position
   * this.check().then(added => {
   *   if (added) {
   *     console.log('New views were added to maintain continuity');
   *   }
   * });
   *
   * @example
   * // Check with custom offset
   * this.check(100, 200).then(added => {
   *   // Custom horizontal and vertical offsets used
   * });
   */
  ...
  addEventListeners(stage) {
    ...
    if (this.isPaginated && this.settings.snap) {
      this.snapper = new _snap.default(
        this,
